...
void BST<T>::Insert(T n) {
    ...
    while (tmp) //tmp가 NULL을 가리키면 (삽입할 적절한 위치) 반복 종료
    {
        ...
        {
            prev = tmp; //prev가 tmp를 가리키도록 한 뒤 tmp 이동
            tmp = tmp->left;
        }
    ...
    if (n < prev->data) //삽입할 n이 prev의 data보다 작을 경우
        prev->left = node; //새로운 노드 삽입
    else if (n > prev->data) //삽입할 n이 prev의 data보다 클 경우
        prev->right = node; //새로운 노드 삽입
}
...
void BST<T>::Delete(T n) {
    ...
    while (del != NULL) //del이 NULL을 가리키면 반복 종료
    {
        ...
        {
            delParent = del;
            del = del->left; //왼쪽 자식노드를 가리키게 함
        }
        else if (n > del->data) //삭제할 n이 del의 data보다 클 경우
        {
            delParent = del;
            del = del->right; //오른쪽 자식 노드를 가리키게 함
        }
    } //삭제할 노드를 찾아 del로 가리키는 과정
    ...
    else if (del->Get_Child() == 1) //del이 자식노드를 1개 가질 경우
    {
        if (del == delParent->left) //del이 delParent의 왼쪽 자식노드인 경우
        {
            if (del->left != NULL) //del이 왼쪽 자식노드를 가질 경우
                delParent->left = del->left; //del이 왼쪽 자식 노드를 가리키게 함
            else if (del->right != NULL) //del이 오른쪽 자식노드를 가질 경우
                delParent->left = del->right; //del이 오른쪽 자식 노드를 가리키게 함
        }
        else if (del == delParent->right) //del이 delParent의 오른쪽 자식노드인 경우
        {
            if (del->left != NULL) //del이 왼쪽 자식노드를 가질 경우
                delParent->right = del->left; //del이 왼쪽 자식 노드를 가리키게 함
            else if (del->right != NULL) //del이 오른쪽 자식노드를 가질 경우
                delParent->right = del->right; //del이 오른쪽 자식 노드를 가리키게 함
        }
    ...
    else if (del->Get_Child() == 2) //del이 자식노드를 2개 가질 경우
    {
        ...
        {
            tmpParent = tmp; //tmpParent가 tmp를 가리키게 한 뒤 tmp 이동시키는 방식
            tmp = tmp->right; //tmp가 오른쪽 자식노드로 이동
        } //작은 값들 중 가장 큰 값을 찾는 과정, 그 값을 tmp가 가리킴

        if (del == tmpParent) //예외처리 필요한 케이스
        {
            tmp->right = del->right; // 삭제할 노드의 자식노드를 임시 노드의 자식노드로 이동시킴
        }
        else //위의 경우 외 모든 경우
        {
            tmpParent->right = tmp->left; //tmp의 자식 노드의 위치 변경
            tmp->left = del->left;
            tmp->right = del->right; //tmp가 삭제할 노드의 자식노드를 자식노드로 받게 함

        }


        if (del == delParent->left) //del이 delParent의 왼쪽 자식노드인 경우
            delParent->left = tmp; //지우고자 하는 노드 위치에 tmp가 가도록 함
        else if (del == delParent->right) //del이 delParent의 오른쪽 자식노드인 경우
            delParent->right = tmp; //지우고자 하는 노드 위치에 tmp가 가도록 함
